...
class Favorite_Controller with ChangeNotifier {
  ...
  delete_Favorite(context, Favorite_Controller pro_Favorite, Profile_Provider pro_Profile, String token, String placeId) async {
    notifyListeners();
    await class_Favorite_api().Delete_Favorite_api(context, token.toString(), placeId);

    await pro_Profile.fetch_Profile(context, pro_Favorite, token);
  }
}
