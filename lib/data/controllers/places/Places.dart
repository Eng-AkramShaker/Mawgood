...
class Places_Controller with ChangeNotifier {
  ...
  Send_Rating(context, String token, String index_id, String show_id, String rating) async {
    print(index_id);
    print(show_id);

    await Rating_Api().Send_Rating_Api(context, token, show_id, rating);

    await fetch_one_item(context, token, index_id, show_id);
  ...
  double parseAvgRating(avgRating) {
    ...
    return parsed != null && parsed.isFinite ? parsed : 1.0;
  }
}
